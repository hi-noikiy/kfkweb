...
function FoodInfo() {
    ...
    this.totalPriceForType = floatMul(1, this.foodprice);
}
...
function addFood(food) {
    ...
    for (var i = 0; i < FoodList.length; i++) {//遍历菜品数组
        if (food.foodID == FoodList[i].foodID) {//找到要增加的对应的菜品
            ...
            FoodList[i].totalPriceForType = floatAdd(parseFloat(FoodList[i].totalPriceForType), parseFloat(FoodList[i].foodprice));
...
function subFood(food) {
    ...
    for (var i = 0; i < FoodList.length; i++) {//遍历菜品数组
        if (food.foodID == FoodList[i].foodID) {//找到要增加的对应的菜品
            ...
            FoodList[i].totalPriceForType = floatSub(parseFloat(FoodList[i].totalPriceForType), parseFloat(FoodList[i].foodprice));
            //kcbCarInFo.totalPrice = parseFloat(kcbCarInFo.totalPrice) - parseFloat(FoodList[i].foodprice);
