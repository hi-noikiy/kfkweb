	...
	$.extend($.XYTipsWindow,{
		...
		init: function (o){
			...
			if(o.___showbg != "" && o.___showbg == true){
				var $boxBgDom = "<div id=\"XYTipsWindowBg\" style=\"position:absolute;background:#999;filter:alpha(opacity=0);opacity:0;width:100%;left:0;top:0;z-index:870617\"><iframe src=\"about:blank\" style=\"width=100%;height:"+$(document).height()+"px;filter:alpha(opacity=0);opacity:0;scrolling=no;z-index:870610\"></iframe></div>";
		...
		showBox: function(o) {
			...
			var $box = $("#"+o.___boxID);
				$box.css({
					position:"relative",
					width:$width+12+"px",
					height:$height+$titleHeight+$borderHeight+$boxDialogHeight+11+"px",
					zIndex: "891208"
				});
			...
			var $boxWrap = $(".___boxWrap",$box);
				$boxWrap.css({
					...
					margin:"0 auto",
					width:$width+"px",
					height:$height+$titleHeight+$boxDialogHeight+7+"px",
		...
		setBoxzIndex: function (o) {
			...
			$box.die().live("click",function(){
				for(var i=0; i < $XYTipsWindowarr.length; i++){
					$XYTipsWindowarr[i].style.zIndex = 891218;
				};
