...
function getCartInfo() {
    ...
    if (FoodList.length == 0) {//菜品数量为0
        ...
        for (var i = 0; i < FoodList.length; i++) {
            ...
            var foodStr = '<tr  food-id=' + FoodList[i].foodID + '><td><p class="food_name">' + foodName + '</p></td><td><div class="order_num">   <a class="minus">&nbsp;</a>   <span>' + FoodList[i].number + '</span><a class="add">&nbsp;</a> </div></td><td>' + FoodList[i].totalPriceForType + '</td><td><a class="del">删除</a></td></tr>';
        ...
        if (deliveryType != 0) {
            $("#delivery").show();
            if (deliveryType == 2) {
                if (totalPrice < parseFloat(freeDeliveryFeePrice)) {
                    $("#delivery .food_name").html("配送费" + parseFloat(deliveryFee) + "元（满" + parseFloat(freeDeliveryFeePrice) + "元免配送费）");
                    $("#delivery").show();
                    $("#delivery .delivery_fee").html(parseFloat(deliveryFee));
                    //如果小于免收额
                    totalPrice += parseFloat(deliveryFee);
                } else {
                    $("#delivery").hide();
                }
            }
            if (deliveryType == 1) {
                $("#delivery .food_name").html("配送费" + parseFloat(deliveryFee) + "元");
                $("#delivery .delivery_fee").html(parseFloat(deliveryFee));
                totalPrice += parseFloat(deliveryFee);
            }
...
function insertIntoCart(foodID, foodName, foodPrice, supName, supID, businessstate, businessmode, sendPrice, activityState) {
    ...
    if (tools.IsEmpty($.cookie("KcbCarInfo"))) {
        ...
        kcbinfo.IsSupportReservation = $("#isSupportReservation").val();
    ...
    food.totalPriceForType = floatMul(1, foodPrice);
...
function selectFood($obj) {
    ...
    var isSupportReservation = $("#isSupportReservation").val();
    ...
    //根据餐厅状态判断
    if (Number(businessstate) == 1 || Number(businessmode) == 3 || Number(isSupportReservation) == 1) {
...
$(document).ready(function () {
    ...
    var businessState = Number($("#businessState").val()), //显示餐厅状态提醒
        ...
        isSupportReservation = Number($("#isSupportReservation").val()),
    ...
    $(".foodListItem").live("click", function (e) {
        var $triggerFood = $(e.target); //获取触发事件来自哪个菜品
        for (k = 0; k < 10; $triggerFood = $triggerFood.parent()) {
            if ($triggerFood.hasClass("foodListItem")) {//点餐
                selectFood($triggerFood.find(".addToOrder").eq(0));
                break;
            }
            else {
                //break;
            ...
            //                break;
            //            }
            //            else if ($triggerFood.hasClass("foodListItem")) {
            //                selectFood($triggerFood.find(".addToOrder").eq(0));
            //                break;
            //            }
        }
    ...
    $("#createOrder #ComfirmOrder").click(function () {
        if (checkBasketFood()) {
            location.href = "order_checkout.html";
        }
    });
...
function checkBasketFood() {
    ...
    if (typeof (kcbCarInFo) == "undefined" || kcbCarInFo == null) {//判断购物车是否为空
        ...
        $("#tbbasket").hide();
        //        $("#no_send_inner").html("美食筐是空的！");
        $("#ComfirmOrder").text("去买单>");
        $("#ComfirmOrder").css("background-color", "#fdda33").css("color", "#333");
        return false;
    }

    var sendPrice = kcbCarInFo.SendPrice;
    //var kcbCarInFo = eval('(' + $.cookie("KcbCarInfo") + ')'); //取得购物车cookie
    var orderPrice = parseFloat(kcbCarInFo.totalPrice);
    if (parseFloat(sendPrice) > parseFloat(orderPrice)) {
        //订单未到起送价
        var deliveryType = $("#deliveryFeeType").val();
        $("#ComfirmOrder").text("还差" + (parseFloat(sendPrice) - parseFloat(orderPrice)) + "元起送");
        $("#ComfirmOrder").css("background-color", "#E5E5E5").css("color", "#A0A0A0");
        if (deliveryType != 0) {
            $("#no_send_inner").html("抱歉，起送价不包含配送费，订单满" + sendPrice + "元起送");
        } else {
