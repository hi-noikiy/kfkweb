$(function() {
    try {
        ...
        if (wxIPhoneControul != "" && "true" == wxIPhoneControul) {
            ...
            iframe.src = $("#wxToClient").attr("href");
            setTimeout(function() {
                try {
                    var after = new Date();
                    if (after - before > 250) {
                        var click = new MPing.inputs.Click("MVirtual_ProductDetail_Expose");
    ...
    $("#cartyuyue").bind("click", function() {
        jQuery.ajax({url: urlEncode("/ware/isAppoint.json", $("#sid").val()), data: {wareId: $("#currentWareId").val()}, success: function(d) {
                try {
                    ...
                    window.location.href = "login.html?sid=" + sid + "&returnurl=http://m.jd.com/product/" + $("#currentWareId").val() + ".html"
                }
            }, error: function(d) {
                window.location.href = "login.html?sid=" + sid + "&returnurl=http://m.jd.com/product/" + $("#currentWareId").val() + ".html"
            }})
    ...
    $("#attention").bind("click", function() {
        if ($("#attention").hasClass("on")) {
            jQuery.ajax({url: urlEncode("/user/cancelFavorite.json", $("#sid").val()), data: {wareId: $("#currentWareId").val()}, success: function(d) {
                    try {
                        var obj = eval("(" + d + ")");
                        $("#attention").removeClass("on");
                        iconShow(1)
                    } catch (e) {
                        window.location.href = "login.html?sid=" + $("#sid").val() + "&returnurl=http://m.jd.com/product/" + $("#currentWareId").val() + ".html"
                    }
                }, error: function(d) {
                    window.location.href = "login.html?sid=" + $("#sid").val() + "&returnurl=http://m.jd.com/product/" + $("#currentWareId").val() + ".html"
                }});
            pingClick("MProductdetail_AddtoFollowed", $("#currentWareId").val(), "", $("#currentWareId").val())
        } else {
            jQuery.ajax({url: urlEncode("/user/addFavorite.json", $("#sid").val()), data: {wareId: $("#currentWareId").val()}, success: function(d) {
                    try {
                        var obj = eval("(" + d + ")");
    ...
    $(".row01").each(function() {
        $(this).bind("click", function() {
            if ($(this).parent(".list-entry").hasClass("open")) {
                $(this).parent(".list-entry").removeClass("open")
            } else {
                $(this).parent(".list-entry").addClass("open");
                if ($(this).attr("id") == "promotionitem" && $("#suite-slider").length == 1) {
                    if (!$("#suite-slider").attr("opend")) {
                        $("#suite-slider").jdSlider({lineNum: 1});
    ...
    var yanBaotick = function() {
        $("span[data=yanbao]").bind("click", function() {
            if ($(this).children("i").hasClass("active")) {
                $(this).children("i").removeClass("active")
            } else {
                $(this).children("i").addClass("active");
                $(this).parent("li").siblings().find("i").removeClass("active")
            }
            var ybId = "";
            $("#ybShow").empty();
            var yanbaoHtml = "";
            $('span[data="yanbao"]').each(function() {
                if ($(this).children("i").hasClass("active")) {
                    ybId += $(this).attr("name") + "@@";
                    yanbaoHtml += '<li class="list-item"><span class="fore01">' + $(this).next("span").text() + '</span><span class="fore02">' + $(this).children("span").text() + "</span></li>"
                }
            });
    ...
    function detail(v) {
        ...
        $.post("/ware/view.json", {wareId: v, sid: $("#sid").val(), provinceId: $("#province").val(), cityId: $("#city").val(), countryId: $("#country").val(), townId: $("#town").val()}, function(d) {
            yuyueRender(d);
            imageRender(d);
            infoRender(d);
            stockRender(d);
    ...
    var url = "http://chat1.jd.com/api/checkChat?pid=" + pid + "&entry=m_item&returnCharset=utf-8&callback=?";
    ...
    $.post("/cart/cartNum.json", {sid: $("#sid").val()}, function(data) {
...
function provinceChange(a) {
    $("#spinner1").show();
    spinner1.spin($("#spinner1")[0]);
    $.post("/ware/view.json", {provinceId: a, sid: $("#sid").val(), wareId: $("#currentWareId").val()}, function(b) {
        stockRender(b);
        infoRender(b);
        spinner1.stop();
        $("#spinner1").hide()
...
function cityChange(b, a) {
    $("#spinner1").show();
    spinner1.spin($("#spinner1")[0]);
    $.post("/ware/view.json", {provinceId: a, cityId: b, sid: $("#sid").val(), wareId: $("#currentWareId").val()}, function(c) {
        stockRender(c);
        infoRender(c);
        spinner1.stop();
        $("#spinner1").hide()
...
function countryChange(b, a, c) {
    $("#spinner1").show();
    spinner1.spin($("#spinner1")[0]);
    $.post("/ware/view.json", {provinceId: a, cityId: c, countryId: b, sid: $("#sid").val(), wareId: $("#currentWareId").val()}, function(e) {
        stockRender(e);
        infoRender(e);
        spinner1.stop();
        $("#spinner1").hide()
...
function townChange(b, a, d, c) {
    $("#spinner1").show();
    spinner1.spin($("#spinner1")[0]);
    $.post("/ware/view.json", {provinceId: a, cityId: d, countryId: c, townId: b, sid: $("#sid").val(), wareId: $("#currentWareId").val()}, function(e) {
        stockRender(e);
        infoRender(e);
        spinner1.stop();
        $("#spinner1").hide()
...
function infoRender(b) {
    $("#ybId").val("");
    $("#currentWareId").val(b.ware.wareId);
    $("#goodName").val(b.ware.wname);
    $("#imgUrl").val(b.ware.imgUrlN5);
    if (b.jshop) {
        $("#jshop").val(b.jshop)
    }
    if (!b.ware.isPop) {
        $("#wareName").html(b.ware.wname + '<i class="icon icon-bg01">\u81ea\u8425</i>')
    } else {
        $("#wareName").html(b.ware.wname)
    }
    $("#wareInfo").attr("href", urlEncode("/detail/" + b.ware.wareId + ".html", $("#sid").val()));
    if (b && b.stock) {
        var a = "";
        if (b.stock.jdPrice && b.stock.jdPrice != "" && b.stock.jdPrice.toLowerCase() != "null" && parseFloat(b.stock.jdPrice) > 0) {
        ...
        var a = "";
        if (b.ware.jdPrice && b.ware.jdPrice != "" && b.ware.jdPrice.toLowerCase() != "null" && parseFloat(b.ware.jdPrice) > 0) {
    ...
    if (b && b.stock && b.stock.flag && b.ware && b.ware.cartFlag) {
        if ((b.stock.jdPrice > 0) || (b.ware.jdprice > 0)) {
    ...
    $("span[data=yanbao]").bind("click", function() {
        ...
        $('span[data="yanbao"]').each(function() {
            if ($(this).children("i").hasClass("active")) {
                d += $(this).attr("name") + "@@";
                c += '<li class="list-item"><span class="fore01">' + $(this).next("span").text() + '</span><span class="fore02">' + $(this).children("span").text() + "</span></li>"
            }
        });
        $("#ybShow").html(c);
        $("#ybId").val(d);
...
function stockRender(c) {
    if (c) {
        $("#provincetip").empty();
        $("#provincetip").append('<select class="fm-select" id="province" ></select>');
        $.each(c.provinceList, function(e, d) {
            if (d.label == c.province.id) {
                $("#provincetip").append('<i id="pShow" style="font-style:normal;">' + c.province.name + "</i>");
                $("#province").append('<option value="' + d.label + '" selected="selected" >' + d.value + "</option> ");
                if (d.label == "84") {
                }
            } else {
                $("#province").append('<option value="' + d.label + '" >' + d.value + "</option> ")
            }
        });
        if (c.city) {
            $("#citytip").empty();
            $("#citytip").append('<select class="fm-select" id="city" ></select>');
            $.each(c.cityList, function(e, d) {
                if (d.label == c.city.id) {
                    $("#citytip").append(c.city.name);
                    $("#city").append('<option value="' + d.label + '" selected="selected" >' + d.value + "</option> ")
                } else {
                    $("#city").append('<option value="' + d.label + '" >' + d.value + "</option> ")
                }
            })
        }
        if (c.country) {
            $("#countrytip").empty();
            $("#countrytip").append('<select class="fm-select" id="country" ></select>');
            $.each(c.countryList, function(e, d) {
                if (d.label == c.country.id) {
                    $("#countrytip").append(c.country.name);
                    $("#country").append('<option value="' + d.label + '" selected="selected" >' + d.value + "</option> ")
                } else {
                    $("#country").append('<option value="' + d.label + '" >' + d.value + "</option> ")
                }
            })
        }
        if (c.town) {
            $("#towntip").empty();
            $("#towntip").append('<select class="fm-select" id="town"></select>');
            $.each(c.townList, function(e, d) {
                if (d.label == c.town.id) {
                    $("#towntip").append(c.town.name);
                    $("#town").append('<option value="' + d.label + '" selected="selected" >' + d.value + "</option> ")
                } else {
                    $("#town").append('<option value="' + d.label + '" >' + d.value + "</option> ")
                }
            })
        }
        ...
        if (c.ware && c.ware.iconList) {
            $.each(c.ware.iconList, function(e, d) {
                $(".promise-ico").append('<span class="txt02"><i class="icon-bg03"><img src="' + d.imageUrl + '" width="15" height="15"/>' + d.name + '</i><span class="txt">' + d.tip + "</span>")
            });
        ...
        if (c.proInformation) {
            ...
            if (c.proInformation.activityList) {
                $.each(c.proInformation.activityList, function(e, d) {
                    $("#saleInfo").append('<li> <span class="col02"> <em class="icon-bg02">' + d.text + '</em> <span class="txt01">' + d.value + "</span> </span> </li>")
                })
            }
            ...
            if (c.proInformation.suitList && c.proInformation.suitList.length > 0) {
                ...
                $("#saleInfo").append('<li> <span class="col02"> <em class="icon-bg02">\u4f18\u60e0\u5957\u88c5</em> <span class="txt01">\u6700\u9ad8\u7701' + b[0].discount + '\u5143</span> </span> <span class="txt-amount"><a href="javascript:void(0)">\u5171' + b.length + "\u6b3e</a></span> </li>");
                var a = "";
                $.each(c.proInformation.suitList, function(e, d) {
                    a += '<li class="icon-suit jd-slider-item"> <a href="' + urlEncode("/suit/" + c.ware.wareId + ".html?resourceType=" + $("#resourceType").val() + "&resourceValue=" + $("#resourceValue").val(), $("#sid").val()) + '"><span class="val-mid">\u5957\u88c5' + (e + 1) + "</span> </a></li>";
                    a += '<li class="list-suit-item  jd-slider-item"> <a href="' + urlEncode("/suit/" + c.ware.wareId + ".html?resourceType=" + $("#resourceType").val() + "&resourceValue=" + $("#resourceValue").val(), $("#sid").val()) + '" class="box-img-link"><div class="p-img"> <img width="70" height="70" alt="img" src="' + c.proInformation.domain + d.mainSkuPicUrl + '"></div></a></li>';
                    $.each(d.productList, function(g, f) {
                        a += '<li class="list-suit-item  jd-slider-item"> <a href="' + urlEncode("/suit/" + c.ware.wareId + ".html?resourceType=" + $("#resourceType").val() + "&resourceValue=" + $("#resourceValue").val(), $("#sid").val()) + '" class="box-img-link"><div class="p-img"> <img width="70" height="70" alt="img" src="' + c.proInformation.domain + f.skuPicUrl + '"></div></a></li>'
                    })
                });
                $("#saleSuit").html(a);
                if ($("#saleInfo").parent().css("display") == "block" && $("#suite-slider").attr("opend") == 1) {
                    $("#suite-slider").jdSlider({lineNum: 1})
                }
            }
        }
        if (c.ware.yanBaoInfo && c.ware.yanBaoInfo.length > 0) {
            $("#ybShow").empty();
            $("#ybInfo").empty();
            $("#ybId").val();
            $.each(c.ware.yanBaoInfo, function(e, d) {
                $("#ybInfo").append('<h6 class="box-i-tit">' + d.sortName + '</h6><ul class="list-click" id="ybInfoList' + e + '"></ul>');
                $.each(d.products, function(g, f) {
                    $("#ybInfoList" + e).append('<li><span class="fore01" name="' + f.platformPid + '" data="yanbao"><i class="icon-check"></i>\u5ef6\u4fdd|&yen;<span>' + f.price + '</span></span><span class="fore02">' + f.sortName + "</span></li>")
                })
            })
        }
    }
...
var eTime;
var newAddWare = function(a, e, d, k, b, h, c) {
    eTime = new Date().getTime() / 1000 + 4;
    if (isNotBlank(d) && d) {
        var i = $("#newOrderServer").val();
        if (k) {
            var f = "/cart/addDirOrder.json?wareId=" + a + "&num=" + e + "&suitSkuId=" + a + "&suitSkuNum=" + e + (isNotBlank(d) && d ? "&isAjax=" + d : "") + "&sType=8&ybId=" + k + "&resourceType=" + b + "&resourceValue=" + h + (isNotBlank(c) && (c != "") ? "&sid=" + c : "")
        } else {
            var f = "/cart/addDirOrder.json?wareId=" + a + "&num=" + e + (isNotBlank(d) && d ? "&isAjax=" + d : "") + "&resourceType=" + b + "&resourceValue=" + h + (isNotBlank(c) && (c != "") ? "&sid=" + c : "")
        }
        var j = jQuery.get(f, function() {
            if (i == "true") {
                window.location.href = "/norder/order.action?wareId=" + a + "&enterOrder=true" + (isNotBlank(c) && (c != "") ? "&sid=" + $("#sid").val() : "")
            } else {
                window.location.href = "/order/order.action?wareId=" + a + "&enterOrder=true&yys=false&from=0" + (isNotBlank(c) && (c != "") ? "&sid=" + $("#sid").val() : "")
            }
        })
    } else {
        if (k) {
            var g = "ajax.php?wareId=" + a + "&num=" + e + "&suitSkuId=" + a + "&suitSkuNum=" + e + (isNotBlank(d) && d ? "&isAjax=" + d : "") + "&sType=8&ybId=" + k + "&resourceType=" + b + "&resourceValue=" + h + (isNotBlank(c) && (c != "") ? "&sid=" + c : "")
        } else {
            var g = "ajax.php?wareId=" + a + "&num=" + e + (isNotBlank(d) && d ? "&isAjax=" + d : "") + "&resourceType=" + b + "&resourceValue=" + h + (isNotBlank(c) && (c != "") ? "&sid=" + c : "")
        }
        var j = ajax(g, function(n) {
            var m = JSON.parse(n).sid;
            $("#sid").val(m);
            var o = JSON.parse(n);
            if (o && o.cartJson && o.cartJson.Num) {
                var l = o.cartJson.Num;
                if (parseInt(l) > 0) {
                    if ($("#toCart i").length == 0) {
                        $("#toCart").prepend("<i></i>")
                    }
...
function cartShow() {
    ...
    if (d > 0) {
        ...
        $("#goCart").click(function() {
            $("#_mask").hide();
            $("#cart").hide();
            window.location.href = "cart.html?sid=" + tmpSid
        });
        $("#cart").show()
    } else {
        $("#cart").hide()
