...
class AliyunClient
{
	...
	public $connectTimeout = 3000;//3�?

	public $readTimeout = 80000;//80�?
	...
	public function execute($request)
	{
		...
		//返回�ЄHTTP文本不是�݇���JSON或者XML，记下错误日�?
	...
	{
	    // 使用urlencode编码后，�?+","*","%7E"���替换即满足 API规定�Є编���规��?
	    $res = urlencode($str);
	...
	protected function computeSignature($parameters, $accessKeySecret)
	{
	    ...
	    // 生成用于计算�о名�Є字符串 stringToSign
	    $stringToSign = 'GET&%2F&' . $this->percentencode(substr($canonicalizedQueryString, 1));
	
	    // 计算�о名，注�׏accessKeySecret后���要加上字�?&'
    ...
    public function curl($url, $postFields = null)
	{
		...
		if (is_array($postFields) && 0 < count($postFields))
		{
			...
			{
				if("@" != substr($v, 0, 1))//判断是不是文件上�?
				{
					$postBodyString .= "$k=" . urlencode($v) . "&"; 
				}
				else//文件�¦��用multipart/form-data，否则用www-form-urlencoded
				{
					$postMultipart = true;
				}
			}
