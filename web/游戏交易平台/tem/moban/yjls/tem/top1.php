  ...
  <div class="menu fontyh">
   ...
   <div class="menun fontyh" id="leftmenu" style="display:none;">
	...
	<? $i=1;while1("*","yjcode_type where admin=1 order by xh asc");while($row1=mysql_fetch_array($res1)){
		$ntp="upload/type/type1_".$row1[id].".png";?>
        <div class="menu1" id="yhmenu<?=$i?>" onmouseover="yhmenuover(<?=$i?>)" onmouseout="yhmenuout(<?=$i?>)">
     <ul class="lu1" style="background:url(<?=$ntp?>) no-repeat;background-position:28px 12px;">
     <li class="l1"><?=$row1[type1]?></li>
     </ul>
     <span class="rx"></span>
     <div class="rmenu rmenu1" style="display:none;margin-top:-<?=$row1[ej]?>px;min-height:214px;" id="rmenu<?=$i?>">
